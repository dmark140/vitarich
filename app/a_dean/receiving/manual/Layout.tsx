 ...
 *
 * A comprehensive form component for manual receiving of hatchery eggs with approval workflow.
 * Handles collection of header information (delivery details), line items (individual egg records),
 * and footer information (logistics details).
 *
 * @component
 * @returns {JSX.Element} A form containing header fields, a dynamic line items table, and footer fields
 *
 * @example
 * ```tsx
 ...
 *
 * State Management:
 * - `header` - Main document record containing delivery metadata
 * - `items` - Array of DraftItem records for individual egg batches
 * - `footer` - Logistics information (crates, trays, vehicle details)
 * - `selectedRows` - IDs of rows selected for deletion
 * - `ItemMaster` - Available egg SKU options filtered for receiving
 * - `farms` - List of breeder farms available for selection
 * - `defaultFarm` - User's default farm setting
 *
 * Key Functions:
 * - `addRow()` - Adds a new blank line item to the table
 * - `updateItem()` - Updates fields in a specific line item
 * - `removeSelectedRows()` - Deletes selected new rows
 * - `validateLineItems()` - Validates required fields before submission
 * - `insertMe()` - Submits the complete record with confirmation dialog
 * - `handleSubmit()` - Form submission handler with validation
 *
 * Features:
 * - Dynamic line item management with add/remove functionality
 * - Breeder reference number auto-generation from house number
 * - Custom age picker with weeks and days granularity
 * - Production date range selection
 * - Item SKU selection with automatic UoM population
 * - Session refresh and authentication checks
...
export default function ApprovalDecisionForm() {
  ...
  useEffect(() => {
    ...
    const init = () => {
      ...
      const filterd = items.filter(
        (items: any) => items.item_group === "receiving_egg",
      );
      console.log({ filterd });
  ...
  const headerFieldsRight = [
    ...
    },
    {
      ...
      onChange: (v: string) =>
        setHeader((h) => (h ? { ...h, shipped_via: v } : h)),
    },
    {
      required: true,
      disabled: false,
    ...
    },
    {
      ...
      label: "DR No",
      value: header?.dr_num || "",
      onChange: (v: string) => setHeader((h) => (h ? { ...h, dr_num: v } : h)),
    },
    {
      required: false,
      disabled: false,
          ...
          <div className="sm:grid md:grid-cols-3 sm:grid-cols-2 gap-6">
            ...
            {headerFieldsLeft.map((field, i) => (
              <div key={i} className="mt-1">
                <Label required={field.required} className="pb-2 mt-1">
                  {field.label}
                </Label>
                <Input
          ...
          <div className="grid grid-cols-5 gap-6">
            <div>
              ...
              <Input
                ...
                value={footer.crates}
                onChange={(e) =>
                  setFooter((f) => ({ ...f, crates: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                value={footer.trays}
                onChange={(e) =>
                  setFooter((f) => ({ ...f, trays: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                onChange={(e) =>
                  setFooter((f) => ({ ...f, van_plate: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                value={footer.driver}
                onChange={(e) =>
                  setFooter((f) => ({ ...f, driver: e.target.value }))
                }
              />
            </div>
            <div>
              ...
              <Input
                ...
                value={footer.serial}
                onChange={(e) =>
                  setFooter((f) => ({ ...f, serial: e.target.value }))
                }
              />
            </div>
          </div>
