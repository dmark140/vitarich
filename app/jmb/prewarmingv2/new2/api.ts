...
export async function listHatchClassiRefs(): Promise<HatchClassiRefOption[]> {
  const { data, error } = await db
    .from("view_prewarming_egg_ref_no")
    .select("egg_ref_no")
    .order("egg_ref_no", { ascending: true });

  if (error) throw error;
  return (data ?? []).filter((r) => !!r.egg_ref_no);
}
...
export async function getEggPreWarmingById(
  id: number,
): Promise<EggPreWarming | null> {
  const { data, error } = await db
    .from("egg_pre_warming")
    .select("*")
    .eq("id", id)
...
export async function createEggPreWarming(
  payload: EggPreWarmingInsert | EggPreWarmingInsert[],
): Promise<EggPreWarming | EggPreWarming[]> {
  const isArray = Array.isArray(payload);

  const query = db.from("egg_pre_warming").insert(payload).select("*");
...
export async function updateEggPreWarming(
  id: number,
  payload: EggPreWarmingUpdate,
): Promise<EggPreWarming> {
  const { data, error } = await db
    .from("egg_pre_warming")
    .update(payload)
    .eq("id", id)
