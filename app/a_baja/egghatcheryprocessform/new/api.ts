...
export async function createEggHatcheryProcess(
  payload: EggHatcheryProcessCreate,
) {
  const { data, error } = await db
    .from(TABLE)
    .insert([payload])
...
export async function updateEggHatcheryProcess(
  id: number,
  payload: EggHatcheryProcessUpdate,
) {
  const { data, error } = await db
    .from(TABLE)
    .update({
